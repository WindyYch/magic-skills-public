...
`video-production-planner` 是这套 AI 视频生产链路的总入口。它负责理解用户目标、选择工作模式、按阶段调度各个 role skill，并用一组固定协议文件把“故事、镜头、执行、字幕、视频合成、QA”串成一条可追踪、可恢复、可局部修订的生产流程。
...
| Concrete execution 层 | `video-asset-executor` + `magicclaw-generate-tts` / `magicclaw-generate-img` / `magicclaw-imgs-to-img` / `magicclaw-generate-video` / `magicclaw-generate-music` | 负责真正把任务执行成媒体资产 |
| 消费与校验层 | `video-subtitle-alignment`、`magicclaw-compose-video`、`video-qa` | 负责消费执行真相并完成字幕、视频合成和验收 |
...
flowchart TD
    ...
    M --> P["magicclaw-compose-video"]
    O --> P
    P --> Q["video-orchestrator-param.json + compose-video-result.json"]
    Q --> R["video-qa"]
```
...
| `magicclaw-compose-video` | 产出 `video-orchestrator-param.json`、`compose-video-result.json` | 不跳过素材或字幕真相直接提交视频合成 |
...
| 资产生成子 agent | `video-asset-dag`、`video-asset-executor` | `magicclaw-generate-tts`、`magicclaw-generate-img`、`magicclaw-imgs-to-img`、`magicclaw-generate-video`、`magicclaw-generate-music` | `asset-dag.json`、`asset-manifest.json`、`run-report.json` |
| 剪辑子 agent | `video-editor` | `video-subtitle-alignment`、`magicclaw-compose-video` | `edit-plan.json`、`subtitle-alignment.json`、`video-orchestrator-param.json`、`compose-video-result.json` |
...
- 剪辑子 agent：只写 `edit-plan.json`、`subtitle-alignment.json`、`video-orchestrator-param.json`、`compose-video-result.json`
...
| 模式 | 适用场景 | 音频主路径 | 字幕规则 |
|---|---|---|---|
| `narration_only` | 解说、旁白故事、说明型视频 | `remotion_tts` 规划路径；执行时 `voiceover_tts` 通常由 `magicclaw-generate-tts` 落地 | 通常不需要 `subtitle-alignment.json` |
...
| 策略 | 含义 |
|---|---|
| `specs_only` | 只产协议，不执行媒体生成，不提交视频合成 |
| `execute_assets` | 产协议并执行媒体素材，但不一定继续提交最终视频合成 |
| `render_final` | 完整跑到视频合成阶段，输出 `compose-video-result.json` 中的 `video_url` |
...
| `compose-video-result.json` | 视频合成真相 |
...
7. 若需要执行媒体生成，调用 `video-asset-executor`。executor 必须先根据 `asset-dag.json` 立即初始化 `asset-manifest.json` 和 `run-report.json`；`asset-manifest.json` 只写资产条目，不写顶层 `tasks`，也不写任何任务状态；`run-report.json` 写入初始 `status: pending` 和 `provider_task_id: null`，接口提交成功后把 provider 返回的 `task_id` 写入 `provider_task_id`，再用它查询和更新任务状态。
8. 若存在 native dialogue 场景，调用 `video-subtitle-alignment` 产出 `subtitle-alignment.json`。
9. 当素材真相和字幕真相满足条件时，调用 `magicclaw-compose-video`。先组装 `video-orchestrator-param.json`，再提交 / 查询合成任务，并把顶层 `task_id`、`status`、`video_url` 等结果持久化到 `compose-video-result.json`。
...
|---|---|---|
| `voiceover_tts` | `magicclaw-generate-tts` | 默认异步提交旁白 TTS，后续用 `provider_task_id` 查询落地 URL |
| `character_reference` | `magicclaw-generate-img` | 角色参考图 |
| `keyframe_image` with reusable character / key-prop refs | `magicclaw-imgs-to-img` | 把角色或关键道具参考图明确合成进关键帧 |
| `keyframe_image` without those refs | `magicclaw-generate-img` | 普通场景关键帧 |
| `image_to_video` | `magicclaw-generate-video` | 图片转视频 |
| `native_dialogue_video` | `magicclaw-generate-video` | 原生对话视频 |
| `bgm` | `magicclaw-generate-music` | 生成背景音乐 |

### 6.2 一条重要原则

- Planner 不直接调用 `magicclaw-generate-tts` / `magicclaw-generate-img` / `magicclaw-imgs-to-img` / `magicclaw-generate-video` / `magicclaw-generate-music`。
...
| `edit-plan.json` | `video-editor` | `storyboard.json` | 时间线、source strategy、motion、audio strategy、subtitle strategy | `video-asset-dag`、`magicclaw-compose-video` |
...
| `asset-manifest.json` | `video-asset-executor` | `asset-dag.json`、已生成或已提供素材 | 最终资产 ID、远程媒体 URL、素材类型，以及必要时的补充元数据；不包含顶层任务清单和任务状态 | `video-subtitle-alignment`、`magicclaw-compose-video`、`video-qa` |
| `run-report.json` | `video-asset-executor` | `asset-dag.json`、执行结果 | 每个任务的实时 status、attempts、provider/API returned task ID、output asset IDs、blocked reason、error、retry state | Planner、`magicclaw-compose-video`、`video-qa` |
| `subtitle-alignment.json` | `video-subtitle-alignment` | `storyboard.json`、`edit-plan.json`、`asset-manifest.json`、可选 `run-report.json` | 对话字幕 cue、对齐状态、blocked scene、对齐所依赖的 source asset truth | `magicclaw-compose-video`、`video-qa` |
| `video-orchestrator-param.json` | `magicclaw-compose-video` | `edit-plan.json`、`asset-manifest.json`、可选 `subtitle-alignment.json` | canonical `video-orchestrator` 入参对象：`job_kind`、`project`、`timeline`、`assets`、`subtitles`、`render_options` | `magicclaw-compose-video` 自身、调试 |
| `compose-video-result.json` | `magicclaw-compose-video` | `video-orchestrator-param.json` 或已有 `task_id` | 提交/查询后的 `task_id`、`status`、`status_code`、`video_url`、`trace_id`、`error` | Planner、`video-qa`、用户 |
...
| `edit-plan.json` | 是合成时间线真相，不是建议稿 |
| `asset-dag.json` | 只定义执行任务，不直接执行工具 |
| `asset-manifest.json` | 必须在 `asset-dag.json` 生成后立即初始化，只写 `assets` 资产条目，不写顶层 `tasks`，也不写任何任务状态；同时完整列出本次执行声明/产生的图片、音频、视频等资产；默认应以远程 `url` 作为主字段，而不是只写本地 `path`；资产 key 应使用 `expected_outputs` 中的最终资产 ID，如 `VID_S01`，而不是 `T_VID_S01` |
| `run-report.json` | 必须在 `asset-dag.json` 生成后立即初始化，写入每个 DAG 任务的 `task_id`、`status: pending`、`provider_task_id: null`、`expected_outputs` 和 `attempts: 0`；接口提交成功后将 provider 返回的 `task_id` 写入 `provider_task_id`，执行过程中同步更新 `pending`、`running`、`success`、`blocked`、`failed`、`skipped`，并显式区分顶层 `initialized`、`running`、`success`、`partial`、`blocked`、`failed` |
| `subtitle-alignment.json` | 不能根据脚本文字长度猜字幕时间 |
| `video-orchestrator-param.json` | 必须保持 canonical `video-orchestrator` 形状；即便 skill 已切到 `magicclaw-compose-video`，当前 payload 里的 `input_protocol` 仍应写 `video_remotion_renderer`，除非 compose API 契约本身变化 |
| `compose-video-result.json` | 必须以顶层 `task_id` / `status` / `video_url` 作为合成真相，不能靠 param 文件存在默认成功 |
...
| `subtitle-alignment.json` | `storyboard.json` + `edit-plan.json` + `asset-manifest.json` |
| `video-orchestrator-param.json` | `edit-plan.json` + `asset-manifest.json` |
| `video-orchestrator-param.json` | 若需要字幕对齐，再加 `subtitle-alignment.json` |
| `compose-video-result.json` | `video-orchestrator-param.json` |

### 8.2 继续推进前必须检查的状态

- 若 `run-report.json.status = blocked`，默认不能继续视频合成。
- 若需要字幕对齐但 `subtitle-alignment.json.status = blocked`，默认不能继续视频合成。
- 若 `asset-manifest.json` 缺少 compose-critical scene assets，默认不能继续视频合成。
- 若用户只要求 `specs_only`，后续执行、字幕、视频合成阶段应全部标记为 `not_requested`。
...
| 从素材和时间线合成成片 | 需要字幕对齐时先走 `video-subtitle-alignment`，再走 `magicclaw-compose-video` |
...
```text
Use video-production-planner. Create a 30 second narration-only short. Specs only. Do not generate images, audio, video, subtitles, or submit final composition.
```

### 10.3 从中间状态继续

```text
Use video-production-planner. Continue this project from existing storyboard.json and edit-plan.json. Execute assets when tools are available, then compose the final video if the execution truth is sufficient.
```
