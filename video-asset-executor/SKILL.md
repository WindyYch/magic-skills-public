...
- For MagicClaw `voiceover_tts`, pass through an upstream `voice_id` only if it is a MiniMax system voice ID. If no `voice_id` is present, call `magicclaw-generate-tts` without `--voice-id` and let the helper use its default system voice; do not block solely because TTS voice ID is missing.
- Do not execute or persist invented TTS voice IDs such as `narrator_voice`, `hero_voice`, or `CHAR_01_VOICE`; omit them and use the helper default instead.
