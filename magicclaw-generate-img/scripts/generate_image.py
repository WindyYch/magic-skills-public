#!/usr/bin/env python3
"""Generate images through the MagicClaw async task API."""
...
DEFAULT_MODEL_TYPE = "duomi_img_gen"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Generate an image with the MagicClaw async task API.",
    )
    parser.add_argument("--task-id", help="Existing MagicClaw task ID to query.")
    parser.add_argument("--prompt", help="Prompt text describing the target image.")
    parser.add_argument(
        "--image-url",
        action="append",
        dest="image_urls",
        help="Optional reference image URL. Repeat this flag to send multiple images.",
    )
    parser.add_argument("--model", default=DEFAULT_MODEL, help="Model name sent to MagicClaw.")
    parser.add_argument(
        "--model-type",
        default=DEFAULT_MODEL_TYPE,
        help="MagicClaw model_type for image generation.",
    )
    parser.add_argument(
        "--quality",
        default=DEFAULT_QUALITY,
        help="Image quality value sent to MagicClaw.",
    )
