#!/usr/bin/env python3
"""Generate speech through the MagicClaw async task API."""
...
CREATE_PATH = "/taskapi/v1/task/gen/tts"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Generate TTS audio with the MagicClaw async task API.",
    )
    ...
    parser.add_argument(
        ...
        help="output_format field sent to MagicClaw.",
    )
    parser.add_argument("--source", default=DEFAULT_SOURCE, help="Source tag sent to MagicClaw.")
    parser.add_argument(
        "--stream",
        action="store_true",
        help="Enable streaming mode. The documented default is false.",
    )
    parser.add_argument("--speed", type=float, default=DEFAULT_SPEED, help="Voice speed.")
    parser.add_argument("--vol", type=float, default=DEFAULT_VOL, help="Voice volume.")
    parser.add_argument("--pitch", type=int, default=DEFAULT_PITCH, help="Voice pitch.")
    parser.add_argument(
        "--format",
        default=DEFAULT_AUDIO_FORMAT,
        help="Audio format nested under audio_setting.",
    )
    parser.add_argument(
        "--sample-rate",
        type=int,
        default=DEFAULT_SAMPLE_RATE,
        help="Audio sample rate nested under audio_setting.",
    )
    parser.add_argument(
        "--bitrate",
        type=int,
        default=DEFAULT_BITRATE,
        help="Audio bitrate nested under audio_setting.",
    )
    parser.add_argument(
...
def validate_args(args: argparse.Namespace) -> argparse.Namespace:
    if args.task_id:
        if args.text:
            raise MagicClawApiError("--text cannot be used with --task-id")
        if args.voice_id:
            raise MagicClawApiError("--voice-id cannot be used with --task-id")
        return args
