#!/usr/bin/env python3
"""Generate music through the MagicClaw async task API."""
...
CREATE_PATH = "/taskapi/v1/task/gen/music"
...
DEFAULT_SOURCE = "music-gen-skill"
DEFAULT_MV = "chirp-crow"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Generate music with the MagicClaw async task API.",
    )
    parser.add_argument("--task-id", help="Existing MagicClaw task ID to query.")
    parser.add_argument("--prompt", help="Lyrics or prompt text for music generation.")
    parser.add_argument("--title", help="Song title sent to MagicClaw.")
    parser.add_argument("--style", help="Style text sent to MagicClaw.")
    parser.add_argument(
        "--custom-mode",
        type=int,
        default=0,
        help="custom_mode field sent to MagicClaw.",
    )
    parser.add_argument(
        "--make-instrumental",
        type=int,
        default=0,
        help="make_instrumental field sent to MagicClaw.",
    )
    parser.add_argument("--mv", default=DEFAULT_MV, help="mv field sent to MagicClaw.")
...
def validate_args(args: argparse.Namespace) -> argparse.Namespace:
    if args.task_id:
        for field in ("prompt", "title", "style"):
    ...
    if not args.title or not args.title.strip():
        raise MagicClawApiError("--title is required unless --task-id is provided")
    if not args.style or not args.style.strip():
        raise MagicClawApiError("--style is required unless --task-id is provided")
    return args
...
def main() -> int:
    ...
    try:
        ...
        output["audio_url"] = output.get("source_url")
